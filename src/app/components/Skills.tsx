        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {skillCategories.map((category, categoryIndex) => (
            <div key={categoryIndex} className="flex flex-col items-center">
              {/* Category box */}
              <div
              className="border-3 border-[#87D3F8] bg-[#87D3F8] px-6 md:px-18 py-3 md:py-5 rounded-lg mb-6 w-40 md:w-64 h-14 md:h-18 flex items-center justify-center"
              ...
              <div className="flex flex-wrap gap-3 justify-center max-w-sm">
                {category.skills.map((skill, skillIndex) => (
                  <span
                  key={skillIndex}
                  className="bg-[#002147] border border-[#87D3F8]/90 text-[#87D3F8] text-xs md:text-[16px] px-6 md:px-4 py-2.5 md:py-2.5 rounded"
                >
                    {skill}
                  </span>
                ))}
